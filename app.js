...
const App = () => {
  ...
  const genererMessageSMS = (beneficiaire) => {
    // Sélectionner le modèle approprié selon le mode de paiement
    ...
    // Remplacer {date} par la date de programmation
...
const SettingsTab = ({ apiSettings, setApiSettings, showNotification }) => {
  ...
  const resetMessageTemplate = () => {
    if (confirm('Êtes-vous sûr de vouloir réinitialiser les modèles de message ?')) {
      setFormSettings({
        ...formSettings,
        messageTemplateEspeces: 'Bonjour M. {nom} votre bon de commande numéro {reference} est programmé pour paiement en numéraire. Bien vouloir vous présenter au service de la comptabilité de la Trésorerie générale de Yaoundé 2. Cordialement',
        messageTemplateVirement: 'Bonjour M. {nom}, votre virement No {reference} est disponible dans votre banque. Bien vouloir vous y rendre. Passez une excellente journée.'
      });
      ...
      <form onSubmit={saveSettings} className="space-y-4">
        ...
        <div className="mt-6 border-t pt-6">
          <div className="flex justify-between items-center mb-2">
            <label className="block text-sm font-medium text-gray-700">Modèles de message</label>
          ...
          {showTemplateHelp && (
            <div className="bg-blue-50 p-3 rounded mb-2 text-sm text-blue-800">
              ...
              <ul className="list-disc list-inside space-y-1 pl-2">
                ...
                <li><code className="bg-blue-100 px-1 rounded">{'{date}'}</code> - Date de programmation</li>
              </ul>
            </div>
          )}
          
          {/* Onglets pour choisir le modèle à éditer */}
          <div className="flex border-b mb-4">
            <button
              type="button"
              className={`py-2 px-4 ${activeTemplateTab === 'especes' ? 'border-b-2 border-blue-500 text-blue-600 font-medium' : 'text-gray-600'}`}
              onClick={() => setActiveTemplateTab('especes')}
            >
              Paiement en numéraire
            </button>
            <button
              type="button"
              className={`py-2 px-4 ${activeTemplateTab === 'virement' ? 'border-b-2 border-blue-500 text-blue-600 font-medium' : 'text-gray-600'}`}
              onClick={() => setActiveTemplateTab('virement')}
            >
              Paiement par virement
            </button>
          </div>
          
          {/* Modèle pour paiement en espèces */}
          {activeTemplateTab === 'especes' && (
            <div>
              ...
              <textarea
                id="messageTemplateEspeces"
                name="messageTemplateEspeces"
                value={formSettings.messageTemplateEspeces}
                onChange={handleChange}
                rows={4}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                placeholder="Modèle de message pour paiement en numéraire"
              />
            </div>
          )}
          
          {/* Modèle pour paiement par virement */}
          {activeTemplateTab === 'virement' && (
            <div>
              ...
              <textarea
                id="messageTemplateVirement"
                name="messageTemplateVirement"
                value={formSettings.messageTemplateVirement}
                onChange={handleChange}
                rows={4}
                className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                placeholder="Modèle de message pour paiement par virement"
              />
            </div>
          )}
